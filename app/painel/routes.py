...
from app import db
from app.models import Importacao, Instituicao, Curso, Turma, Aluno, Professor, Disciplina, Nota, Avaliacao, DisciplinaTurmaProfessor
...
def painel():
    ...
    try:
        ...
        # AJUSTADO: Disciplinas por professor via tabela associativa
        disciplinas_por_professor = db.session.query(
            Professor.nome.label('professor'),
            func.count(DisciplinaTurmaProfessor.disciplina_id).label('quantidade')
        ).join(DisciplinaTurmaProfessor, DisciplinaTurmaProfessor.professor_id == Professor.id)\
         .group_by(Professor.id).order_by(desc('quantidade')).limit(10).all()
        ...
        # Labels e valores para gráficos
...
def baixar_todos_templates():
    """Baixa todos os templates em formato .zip"""
    try:
        zip_path = criar_zip_modelos(
            pasta_modelos=os.path.join(current_app.root_path, TEMPLATES_FOLDER),
